<template>
  <div class="whiteip-container">
    <div v-for="(ip, index) in whiteIpList" :key="index" class="ip-container">
...
const fetchWhiteIP = async () => {
  try {
    const response = await axios.get('http://localhost:8000/whiteip');
    whiteIpList.value = response.data;
  } catch (error) {
...
const deleteWhiteIP = async (ip: string) => {
  ...
  try {
    const response = await axios.post('http://localhost:8000/whiteip', req);
    if (response.data) {
      const index = whiteIpList.value.indexOf(ip);
      if (index > -1) {
        whiteIpList.value.splice(index, 1);
      }
      emits('hideOverlay'); // 触发关闭弹窗事件
    }
  } catch (error) {
