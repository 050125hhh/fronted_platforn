    ...
    <el-card shadow="hover" class="traffic-analysis-card">
      <template #header>
        <div class="card-header with-tag">
          <span>历史流量分析</span>
          <!-- <el-tag type="primary" effect="dark">使用模型: {{ modelName || '加载中...' }}</el-tag> -->
        </div>
      </template>
      
      <div ref="ipcontainer" class="ip-container">
        <el-empty v-if="!hasData && !errorMessage" description="暂无历史数据"></el-empty>
...
import HistoryTraffic from '../components/HistoryTrafficGraph.vue';
import MacliousTrafficClassification from '../components/MacliousTrafficClassificationGraphByTime.vue';
...
onMounted(() => {
  ...
  // 每 1 秒轮询一次获取 IP 数据（可调整时间间隔）
  intervalId = setInterval(fetchData, 1000);
  // 每 1 秒轮询一次获取模型名称（可调整时间间隔）
